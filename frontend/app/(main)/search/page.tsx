...
import { Briefcase, Gamepad, Coffee, Utensils, Star, Book, Plane } from "lucide-react";
...
function InterestCard({ name, icon }: { name: string; icon: ReactNode }) {
  return (
    <Link href={`/search/${encodeURIComponent(name)}`}>
      <Card className="flex flex-col items-center justify-center p-4 h-32 w-full rounded-2xl transition-transform hover:scale-105 bg-secondary text-secondary-foreground">
        <div className="text-primary">{icon}</div>
        <p className="font-semibold text-md text-center mt-2">{name}</p>
      </Card>
    </Link>
  );
}
...
export default function SearchPage() {
  return (
    <div className="p-4 space-y-8">
      {/* 3. 내가 설정한 관심사 렌더링 */}
      <div>
        <h2 className="text-2xl font-bold mb-4">나의 관심사 ✨</h2>
